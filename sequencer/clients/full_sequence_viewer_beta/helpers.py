...
def get_sequence_parameters(x):
    ...
    if type(x).__name__ in ['str', 'unicode'] and x[0] == '*':
        ...
        return set(list(chain.from_iterable([get_sequence_parameters(xx) for xx in x])))
    elif type(x).__name__ == 'dict':
        return set(list(chain.from_iterable([get_sequence_parameters(v) for v in x.values()])))
    else:
        return []
