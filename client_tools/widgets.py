...
class SuperSpinBox(QtGui.QLineEdit):
    # num_decimals, display_factor=1):
    def __init__(self, display_range, units, num_decimals=1, significant_figures=3):
        super(SuperSpinBox, self).__init__()
        self.display_range = display_range
        self.units = units
        # display in units of display factor
        self.display_factor = 10.**units[0][0]
        # a string to be appended to the displayed value (e.g. 's')
        self.unit = units[0]
    ...
    def step(self, up):
        ...
        text = self.text().split(
            ' ')[0]  # previously displayed value, strip away unit
        ...
        # what is this in units of display factor = 1?
    ...
    def display(self, value):
        if str(value)[0] == '*':
            self.setText(value)
            return
        # previously displayed value, strip away unit
        ...
        value = sorted([self.display_range[0], value, self.display_range[1]])[
            1]  # force value to be within [min, max]
        self.display_factor, self.unit = self.format_number(value)
        self.setText(str('{:0.'+str(self.num_decimals)+'f}' +
                     ' {}'.format(self.unit)).format(value/self.display_factor))
    ...
    def format_number(self, value):
        ...
        def log10(x):
            if x != 0:
                ...
                return float(2**64-1)
        factor = min(zip(*self.units)
                     [0], key=lambda x: abs(x-np.floor(log10(value))+1))
...
class IntSpinBox(QtGui.QLineEdit):
    ...
    def display(self, value, overwrite=False):
        if overwrite or not self.hasFocus():
            ...
            value = sorted([min(self.displayRange), value,
                           max(self.displayRange)])[1]
