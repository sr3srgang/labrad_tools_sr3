...
#            self.serial_server.write(self.serial_address, command)
    ...
    def set_frequency(self, frequency, board=0, output='low'):
        ...
        if output == 'high':
            ...
            print('will try to set low frequency to ' + str(frequency))        
        command = ''.join(instruction_set)
        response = self.ser.write(command)
        print('freq written. response: ' + str(response))
#        self.serial_server.write(self.serial_address, command)
