...
def add_trace(trace_exc, trace_gnd, trace_bgd, thisAx):
    ...
    thisAx.fill_between(pico_shot_range, 0,
                        trace_gnd[pico_shot_range], alpha=.4, color='grey')
...
def fit_gaussian(freqs, exc):
    ...
    # print(f"cen_guess={cen_guess}, freq range=({min(freqs)},{max(freqs)})")
    freq_range = max(freqs) - min(freqs)
    p0 = [freqs[cen_guess], freq_range*0.2, exc[cen_guess], 0.01]
    fit, cov = curve_fit(gaussian, freqs, exc, p0=p0,
                         bounds=([min(freqs) - freq_range*0.1, 0.0001, -1.5, -0.5],
                                 [max(freqs) + freq_range*0.1, freq_range*2, 1.5, 1.5]))
...
def add_gaussian(freqs, exc, freq_ax, offset=True, inverted=False):
    ...
    freq_ax.set_title("Cen: {:.2f}. FWHM: {:.2f}".format(
        fit[0], FWHM), color='white')
...
def fit_inverted_gaussian(freqs, exc):
    ...
    # print(f"cen_guess={cen_guess}, freq range=({min(freqs)},{max(freqs)})")
    freq_range = max(freqs) - min(freqs)
    p0 = [freqs[cen_guess], freq_range*0.2, exc[cen_guess] - 1, 1]
    fit, cov = curve_fit(gaussian, freqs, exc, p0=p0,
                         bounds=([min(freqs) - freq_range*0.1, 0.0001, -1.5, -0.5],
                                 [max(freqs) + freq_range*0.1, freq_range*2, 1.5, 1.5]))
    FWHM = fit[1]*2*np.sqrt(2*np.log(2))
...
def import_pico_scan(data_path, exp_name, shot):
    ...
    path_last = os.path.join(
        data_path, exp_name, '{}.clock_pico.hdf5'.format(shot - 1))
    f = h5py.File(path)
...
def simple_pico_clock(settings, fig, ax, data_x, data_y):
    ...
    if shot > 3:
        gnd, exc, background, freq = import_pico_scan(
            settings['data_path'], settings['exp_name'], shot)
        gnd_sub = gnd - background
        exc_sub = exc - background
        if settings['isCleanUp'] == False:
            frac = calc_excitation(
                np.sum(gnd_sub[pico_shot_range]), np.sum(exc_sub[pico_shot_range]))
        else:
            # swap if cleanup
            frac = calc_excitation(
                np.sum(exc_sub[pico_shot_range]), np.sum(gnd_sub[pico_shot_range]))
        print(freq)
...
# default_name = "horizontal_mot_{}_horizontal_mot_fluor_vertical_mot_{}_fluorescence.png"
    ...
    modified_shot = settings['shot_number'] - shot_offset
    paths = [os.path.join(settings['data_path'], settings['exp_name'],
                          default_name.format(img, modified_shot)) for img in imgs]
    ...
    exc_sub = process_file(paths[1], ROI=this_roi,
                           background=paths[2], zoom=True)
