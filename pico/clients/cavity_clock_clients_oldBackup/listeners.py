...
from data_analysis.cavity_clock.read_data_oldBackup import *
from data_analysis.cavity_clock.helpers import *
from pico.clients.cavity_clock_clients_oldBackup.params import *
    ...
    if n > 1:
        ...
        for i in np.arange(n_windows):
            if not fixed_ixs[i]:
                ...
                ax2.plot(n, dfs[i], markers_fixed[fixed_counter],
                         color=cs[i], alpha=.1)
                fixed_counter += 1
        data_x.append(n)
        data_y.append(dfs)
...
def atom_number(update, ax, data_x, data_y, bad_points=None, time_domain=False, time_name='sequencer.t_dark', freq_domain=True, add_fit=False):
    ...
    for message_type, message in update.items():
        value = message.get('clock_pico')
        if message_type == 'record' and value is not None:
            gnd, exc, background, freq, _, shot_num, t_dark = get_clock_data(
                value, time_name=time_name)
...
def exc_frac(update, ax, data_x, data_y, time_domain=False, time_name='sequencer.clock_phase', freq_domain=True, add_fit=False, n_avg=1, bad_shot=None):
    ...
    for message_type, message in update.items():
        ...
        if message_type == 'record' and value is not None:
            ...
            if shot_num is None or shot_num < 2:
                return False
            exc_frac = calc_excitation(np.sum(
                (gnd - background)[pico_shot_range]), np.sum((exc - background)[pico_shot_range]))
            if time_domain:
                x_ax = t_dark
...
def get_clock_data_old(path, time_name='sequencer.t_dark'):
    ...
    if shot_num > 0:
        ...
        # MM 080221 keeping up with ever- changing fun lol
        exc = np.array(f_last['gnd'])
        background = np.array(f['bgd'])
        ts = np.array(f['time'])
